...
:set   
    ...
    set jsonbody='{\"jsonrpc\":\"2.0\", \"method\":\"condenser_api.get_discussions_by_created\", \"params\":[{\"tag\":\"hive-13323\",\"limit\":1}], \"id\":1}' 
...
:: Let the user know that Blinkit is going to watch for Upvotes by displaying the text:
	echo %Red%Hive %Grey%Blink on new Splinterlands posts and votes  %Grey%(limit is latest post)
	echo.
...
:: Let the user know, there is a new comment/votes by displaying the text:   
	echo.
	echo %White%Blinkit new Splinterlands community activity detected, %Green%Light blink! %White%
